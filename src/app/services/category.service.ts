...
saveCategory(name){
  return this.http.post(serverUrl+'/category/save',{ categoryName: name})
}

saveItem(idcat,name){
  return this.http.post(serverUrl+'/item/save',{ category_idcategory: idcat, name: name})
}

saveAttribute(iditem,name){
  return this.http.post(serverUrl+'/attribute/save',{itemByItemIdItem: iditem, attributeName: name})
}

deleteCategory(id){
  return this.http.get(serverUrl+'/category/delete/'+id,{responseType: 'text'})
}
